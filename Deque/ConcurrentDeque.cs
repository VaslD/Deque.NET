...
namespace System.Collections.Concurrent
{
    ...
    public class ConcurrentDeque<T> : IConcurrentDeque<T>
    {
        ...
        public void PushRight(T item)
        {
            ...
            while (true)
            {
                ...
                if (anchor._right == null)
                {
                    ...
                    if (Interlocked.CompareExchange(ref _anchor, newAnchor, anchor) == anchor) return;
                }
        ...
        public Boolean TryPopRight(out T item)
        {
            Anchor anchor;
            var    spinner = new SpinWait();
            ...
            while (true)
            {
                ...
                if (anchor._right == anchor._left)
                {
                    ...
                    if (Interlocked.CompareExchange(ref _anchor, newAnchor, anchor) == anchor) break;
                }
                else if (anchor._status == DequeStatus.Stable)
                {
            ...
            if (rightmostNode != null) Interlocked.CompareExchange(ref rightmostNode._right, null, node);
        ...
        /// object removed. If no object was available to be removed, the value is unspecified.</param>
        /// <returns>true if an element was removed and returned successfully; otherwise, false.</returns>
        public Boolean TryPopLeft(out T item)
        {
            Anchor anchor;
            var    spinner = new SpinWait();
            ...
            if (leftmostNode != null) Interlocked.CompareExchange(ref leftmostNode._left, null, node);
        ...
        public Boolean TryPeekRight(out T item)
        {
        ...
        /// of the <see cref="ConcurrentDeque{T}"/> or an unspecified value if the operation failed.</param>
        /// <returns>true if an item was returned successfully; otherwise, false.</returns>
        public Boolean TryPeekLeft(out T item)
        {
        ...
        private void StabilizeRight(Anchor anchor)
        {
            ...
            if (prevNext != newNode)
            {
                ...
                if (Interlocked.CompareExchange(ref prev._right, newNode, prevNext) != prevNext) return;
            }

            /*
             * Try to mark the anchor as stable.
             * This step is done outside of the previous "if" block:
             *   even though another thread may have already updated previous right pointer,
        ...
        private void StabilizeLeft(Anchor anchor)
        {
            ...
            if (prevNext != newNode)
            {
                ...
                //CAS failure means that another thread already stabilized the deque.
                if (Interlocked.CompareExchange(ref prev._left, newNode, prevNext) != prevNext) return;
            }

            /*
             * Try to mark the anchor as stable.
             * This step is done outside of the previous "if" block:
             *   even though another thread may have already updated previous left pointer,
             *   this thread might still preempt the other and perform the second step (i.e., update the anchor).
             */
            var newAnchor = new Anchor(anchor._left, anchor._right, DequeStatus.Stable);
            Interlocked.CompareExchange(ref _anchor, newAnchor, anchor);
        ...
        /// </remarks>
        public IEnumerator<T> GetEnumerator() { return ToList().GetEnumerator(); }
        ...
        Object ICollection.SyncRoot
            => throw new
                   NotSupportedException("The SyncRoot property may not be used for the synchronization of concurrent collections.");
        ...
        /// </remarks>
        Boolean IProducerConsumerCollection<T>.TryTake(out T item) { return TryPopLeft(out item); }
        ...
        internal class Anchor
        {
            ...
            public void Validate()
            {
                ...
                Debug.Assert((_left == null && _right == null) || (_left != null && _right != null));

                //assert that if the anchor is empty, then it is stable
                if (_left == null) Debug.Assert(_status == DequeStatus.Stable);
            }
        }
        ...
        internal class Node
        {
            ...
            internal Node(T value) { _value = value; }
        }
    }
}
