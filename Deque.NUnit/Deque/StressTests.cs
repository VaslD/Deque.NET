...
public class StressTests
{
        ...
        for (Int32 i = 0; i < operationsCount; i++)
        {
            ...
            else
            {
                ...
                case Op.Add:
                    ...
                    break;
                case Op.SetIndex:
                    if (_deque.IsEmpty) goto case Op.Add;
                    ...
                    break;
                case Op.PopLeft:
                    if (_deque.IsEmpty) goto case Op.PushLeft;
                    Assert.DoesNotThrow(() => val = _deque.PopLeft());
                    Assert.AreEqual(_shadow.First(), val);
                    _shadow.RemoveAt(0);
                    break;
                case Op.PopRight:
                    if (_deque.IsEmpty) goto case Op.PushRight;
                ...
                case Op.Remove:
                    ...
                    //draw a random item
                    Int32 index = rnd.Next(_shadow.Count);
                    Int32 item  = _shadow[index];
    ...
    {
        for (Int32 i = 0; i < _shadow.Count; i++) Assert.AreEqual(_shadow[i], _deque[i]);
    }
    ...
    {
        if (_shadow.Count == 0) Assert.True(_deque.IsEmpty);
    }
    ...
    private class RandomOpGenerator
    {
        ...
        public Op Pick()
        {
            Int32 index = _rnd.Next(_distribution.Count);
